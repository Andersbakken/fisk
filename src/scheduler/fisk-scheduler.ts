...
import { setrlimit } from "@andersbakken/fisk-native";
...
if (intervalMs > 0) {
    ...
    if (summaryIntervalMs > 0) {
        setInterval(() => {
            console.log(
                `event-loop-lag summary max=${maxLag}ms stalls=${stalls} over last ${summaryIntervalMs}ms (threshold ${thresholdMs}ms)`
            );
            maxLag = 0;
            stalls = 0;
